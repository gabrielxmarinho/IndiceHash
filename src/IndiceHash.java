...
public class IndiceHash {
	...
	public int funcaoHash(String chaveBusca) {
		...
		//Aponta para um bucket a partir do hash
		//Garantir que fique entre 0 e qtdBuckets-1 [0,nBuckets-1]
	...
	public IndiceHash(String nomeArquivo) {
		...
		try {
			...
			//Taxacolisoes = ncolisoes/nRegistros
			//TaxaBucketOverflow = nBucketOverflow/nBuckets
			System.out.printf("Taxa de Buckets Overflow = %f%%\n",100 * (float) this.nBucketOverflow/this.buckets.length);
			scanner.close();
	...
	public int chaveBusca(String chave) throws Exception {
		int qtd = 0; //Quantidade de pesquisas
